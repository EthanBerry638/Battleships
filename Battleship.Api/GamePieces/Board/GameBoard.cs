...
public class GameBoard : IGameBoard
{
    ...
    public PlacementResult PlaceShip(IShip ship)
    {
        ...
        if (invalidCoordinates.Count != 0) return new PlacementResult(false, invalidCoordinates);

        foreach (Coordinate coordinate in ship.Coordinates) _board[coordinate.X, coordinate.Y].OccupyingShip = ship;
