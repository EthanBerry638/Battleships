...
public class GameBoardTests
{
    ...
    public void AreAllShipsSunk_ShouldReturnFalse_WhenNoShipsArePlaced()
    {
        ...
        bool result = gameBoard.AreAllShipsSunk();
    ...
    public void AreAllShipsSunk_ShouldReturnFalse_WhenShipsArePlacedAndNoneAreSunk()
    {
        ...
        foreach (var ship in ships.Skip(1)) ship.Verify(s => s.IsSunk(), Times.AtMostOnce);
    }
    ...
    public void AreAllShipsSunk_ShouldReturnTrue_WhenShipsArePlacedAndAllAreSunk()
    {
        ...
        foreach (var ship in ships) ship.Verify(s => s.IsSunk(), Times.Once);
    }

    [Theory]
    [MemberData(nameof(MissingShipTestData))]
    public void ValidateFleet_ShouldReturnFailureWithListOfShips_WhenFleetIsMissingShips(
        List<ShipType> presentShipTypes, ShipType missingShipType)
    {
        var gameBoard = new GameBoard();
        ...
        foreach (var ship in ships) gameBoard.PlaceShip(ship);
    ...
    public static IEnumerable<object[]> MissingShipTestData()
    {
        ...
        foreach (var missingType in allShipTypes)
            yield return
            [
                allShipTypes.Where(t => t != missingType)
                    .ToList(),
                missingType
            ];
    }
    ...
    public void ValidateFleet_ShouldReturnFailureWithListOfExtraShips_WhenFleetHasExtraShips(
        List<ShipType> presentShipTypes, ShipType extraShipType)
    {
        var gameBoard = new GameBoard();
        var ships = presentShipTypes.Select((type, index) =>
        {
    ...
    public static IEnumerable<object[]> ExtraShipTestData()
    {
        ...
        foreach (var extraType in allShipTypes) yield return [allShipTypes.Concat([extraType]).ToList(), extraType];
    }
